...
namespace EventBus.RabbitMQ
{
	public class EventBusRabbitMQ : BaseEventBus
	{
		...
		RabbitMQPersistendConnection persistendConnection;
		...
		public EventBusRabbitMQ(EventBusConfig config, IServiceProvider serviceProvider) : base(config, serviceProvider)
		{
			if (config.Connection != null)
			{
				...
				{
					//Self referencing loop detedted for property
			...
			persistendConnection = new RabbitMQPersistendConnection(connectionFactory, config.ConnectionRetryCount);
		...
		private void SubsManager_OnEventRemoved(object? sender, string eventName)
		{
			...
			if (!persistendConnection.IsConnected)
				persistendConnection.TryConnect();
		...
		private IModel CreateConsumerChannel()
		{
			...
			var channel = persistendConnection.CreateModel();
